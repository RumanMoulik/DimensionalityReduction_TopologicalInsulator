...
fig_maxv,ax_maxv = plt.subplots(2,3, figsize=(36,18), sharex=True,sharey=True)
...
for i in range(0,6):
    ...
    ax_maxv[row,col].set_title(graph_titles[i],x=0.5,y=0.01,fontsize=25)

ax_maxv[0,0].set_xticks([1,5,10,15,20],[1,5,10,15,20])    
ax_maxv[0,0].tick_params(axis='both', which='major', labelsize=20)
ax_maxv[1,0].tick_params(axis='both', which='major', labelsize=20)
ax_maxv[1,1].tick_params(axis='both', which='major', labelsize=20)
ax_maxv[1,2].tick_params(axis='both', which='major', labelsize=20)
fig_maxv.savefig("adaboost_max_score.png")
...
plt.ylabel("Time(s)")
plt.legend()
fig_time.savefig("adaboost_time.png")
#plt.show()
