...
for i in searc:
    ...
    els = structure.formula.split(" ")
    #print(els)
    el=[]
    el.append(mp.Element(els[0][:-1]))
    el.append(mp.Element(els[1][:-1]))
    if els[0][-1]=='2' or els[0][-1]=='8': el.append(el[0])
    if els[1][-1]=='2' or els[1][-1]=='8': el.append(el[1])
    #print(el)
    if len(els)==3: el.append(mp.Element(els[2][:-1]))
